package com.testdemo.testDatePicker.datepicker.views;
...
public class MonthView extends View {
    ...
    public boolean onTouchEvent(MotionEvent event) {
        int lengthX = Utils.dp2px(10);
        int lengthY = Utils.dp2px(10);
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                if (isNewEvent) {
                    if (Math.abs(lastPointX - event.getX()) > lengthX) {
                        mSlideMode = SlideMode.HOR;
                        isNewEvent = false;
                    } else if (Math.abs(lastPointY - event.getY()) > lengthY) {
