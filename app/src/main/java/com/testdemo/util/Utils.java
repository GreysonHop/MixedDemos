...
public final class Utils {
    ...
    public static <P> P getGenericInstance(Class<?> subClazz, int i) {
        Type superclass = subClazz.getGenericSuperclass();
        if (superclass instanceof Class)
            return null;

        ParameterizedType parameterized = (ParameterizedType) superclass;
        try {
            return ((Class<P>)parameterized.getActualTypeArguments()[i]).newInstance();
        } catch (Exception e) {
            e.printStackTrace();
        }
