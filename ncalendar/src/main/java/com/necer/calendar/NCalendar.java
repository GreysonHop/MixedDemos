...
public abstract class NCalendar extends FrameLayout implements IICalendar, NestedScrollingParent, ValueAnimator.AnimatorUpdateListener {
    ...
    private boolean isNestedScroll = true;//子View中的滚动（如果可滚动的话）是否会影响日历视图的收展
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        childLayoutLayoutParams.height = getMeasuredHeight() - (isNestedScroll ? weekHeight : (int) childView.getY());
        super.onMeasure(widthMeasureSpec, heightMeasureSpec);
    }
    ...
    protected void gestureMove(float dy, int[] consumed) {
        ...
        } else if (dy < 0 && childViewY <= monthHeight && childViewY >= weekHeight && (!isWeekHoldEnable || consumed == null)
                && (!isNestedScroll || targetView == null || !targetView.canScrollVertically(-1))) {
            //setY  持续过程  周->月的过程
            if (isMonthStretchEnable && realMonthHeight != monthHeight) {
                layoutParams.height = monthHeight;
                monthCalendar.setLayoutParams(layoutParams);
    ...
    public void setDefaultSelectFirst(boolean isDefaultSelectFirst) {
        monthCalendar.setDefaultSelectFirst(isDefaultSelectFirst);
//        weekCalendar.setDefaultSelectFirst(isDefaultSelectFirst);
    }
    ...
    @Override
    public void setNestedScroll(boolean isNestedScroll) {
